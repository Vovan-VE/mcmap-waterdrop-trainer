...
execute at @e[type=minecraft:armor_stand,tag=selectFloor,limit=1] align xyz if block ~-1 ~ ~ minecraft:bedrock at @e[type=minecraft:armor_stand,tag=selectDepth,limit=1] align xyz if block ~-1 ~ ~ minecraft:bedrock run setblock 1 0 0 minecraft:bedrock replace
# check flag block
